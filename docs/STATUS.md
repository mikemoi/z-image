...
- 想法、日志、计划三个文字入口。
- 统一分类：类型、领域、主轴、关联、标签、来源；用途退出核心分类。
...
- “我的 → 数据概览”展示内容、类型、领域、主轴、来源和分类状态。
- 首页只保留搜索、AI 后台状态和“今日推荐”；旧分类入口已迁入集中批阅。
- “分类浏览”和“我的 → 集中批阅”支持按类型、领域、固定主轴、来源查看；集中批阅额外支持常用标签。
...
- 批阅页保留独立重新分类；分类模式按固定主轴树筛选，标签只显示高频前 12 个。
...
- 截图 `source` 默认写“截图”；旧 `theme/use_tag/topics` 不删除。

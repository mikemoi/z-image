...
### 用(方便用)· 四种遇见
- **按分类浏览**:Browse 与集中批阅统一按类型 / 领域 / 主轴 / 来源浏览；ADHD、交易、语言、马德里生活等作为固定主轴入口出现。
- **统一分类**:类型 / 领域 / 主轴 / 关联 / 标签 / 来源。用途退出新分类核心，旧字段不删除。
- **重新遇见**:首页偶尔翻出最久没见的碎片,配原图缩略,看完轮换。无待办、纯偶遇。
- **搜索**:全文检索**全部内容**(截图条目 + 手写文字,标题/摘要/正文,中文走子串兜底)。
- **语义关联**:预留 pgvector(第二刀)。

### 问问 AI · 会给你讲的第二脑(v0.3)
- **详情页「问问 AI」**:你主动点才调用(拉不推、按需付费),结果缓存不重复烧钱。
- 给两样:**看法/定义**(讲明白这是什么,标「AI 补充」与原文分开)、**质量判断**(干货/反面样本/无信息量)。分类统一由自动分类管线和人工编辑维护。

### 记 · 文字入口(v0.3)
- **想法 / 日志 / 计划**,零摩擦记下(`core.entries`,复用 core)。
- **想法**可来自自己或截图详情并可精选入脑；**日志**带日期、按天翻、往年今天、可选心情；**计划**钉住常驻。

### 消化闭环 · 三层存储门槛递减（截图旧主流程）
```
...
| 分组 | 前缀 | 主要能力 |
|---|---|---|
| 条目 | `/api/items` | 上传 / 列表 / 详情 / 改分类与标签 / 问问AI / 清库 / review·promote·落箱 / 软删·恢复·销毁 |
| 文字入口 | `/api/entries` | 记一条 / 待整理 / 计划 / 日志·往年今天 / 归位 / 软删·恢复 |
| 统计 | `/api/stats` | 数据概览；旧维度计数接口保留兼容 |
...
- [`docs/STATUS.md`](docs/STATUS.md) —— 当前实现快照、兼容层、已知问题与下一步
- [`docs/DECISIONS.md`](docs/DECISIONS.md) —— kind/type、theme/domain、删除和人工优先等关键决策
- [`docs/TESTING.md`](docs/TESTING.md) —— 无付费回归测试与本地集成验收

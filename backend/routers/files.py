...
async def get_file(checksum: str, thumb: bool = Query(default=False)):
    ...
    with get_conn() as conn:
        row = conn.execute(
            "SELECT checksum, file_path, original_filename FROM image.files WHERE checksum = %s LIMIT 1",
            (checksum,),
        ).fetchone()
    if not row:
        raise HTTPException(404, "file not found")
    path = path_from_record(row["checksum"], row["file_path"], row["original_filename"])
    if not path.exists():
